...
--Inventory Population Procedures
DROP PROCEDURE generateProductLogItem
--EXEC generateProductLogItem //Creates one random Product at production house level 
DROP PROCEDURE generateProductLogItems
--EXEC generateProductLogItems //Creates 100 random Products at  production house level
DROP PROCEDURE generateInventories
--EXEC generateInventories (numTimes) //Creates numTimes objects at each level from productionhouse to store
